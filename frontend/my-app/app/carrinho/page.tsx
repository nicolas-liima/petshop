...
export default function CarrinhoPage() {
  const { itens, removerItem, atualizarQuantidade, limparCarrinho, total, finalizarPedido } = useCarrinho();
  const { user } = useAuth();
  ...
  const handleQuantidadeChange = (produtoId: number, novaQuantidade: string) => {
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex items-center space-x-2">
                        <label htmlFor={`quantidade-${item.produtoId}`} className="text-sm text-gray-700">
                          Qtd:
                        </label>
                        <input
                          ...
                          onChange={(e) => handleQuantidadeChange(item.produtoId, e.target.value)}
                          className="w-16 px-2 py-1 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                        />
                      </div>
